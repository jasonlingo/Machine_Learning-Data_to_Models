...
class BayesQuery(object):
    ...
    def marginalProb(self, exp):
        ...
        exp2 = self.extractExp(exp)
        node = self.nodes[exp2[0]]
        if not node.hasParent():
            return self.prior[ exp2[0] ][ exp2[1] ]
        
        # if the node has parents, need to sum over all possible query conditioned on its parents
